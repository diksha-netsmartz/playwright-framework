...
test('TC_094: CSM - To verify staff is able to update profile', { tag: ['@CSM', '@CSMMyProfile'] }, async ({ page }) => {
  ...
  // Generate runtime dynamic data for all 3 fields
  const dynamicEmergencyContactName = TestDataGenerator.generateRandomFullName('Emergency');
  const dynamicEmergencyContactPhone = TestDataGenerator.generateRandomPhoneNumber();
  const dynamicLicenseNumber = `${Math.floor(1000000000000000 + Math.random() * 9000000000000000)}`;
  ...
  await test.step('Step 3: Update the fields with runtime generated data', async () => {
    await staffProfilePage.updateProfileDetails({
      homePhone: dynamicHomePhone,
      emergencyContactName: dynamicEmergencyContactName,
      emergencyContactPhone: dynamicEmergencyContactPhone,
      licenseNumber: dynamicLicenseNumber
    });
  });
  ...
  await test.step('Step 6: Navigate to My Profile and verify updated details', async () => {
    await staffHomePage.navigateToMyProfile();
    await staffProfilePage.verifyProfileDetails({
      homePhone: dynamicHomePhone,
      emergencyContactName: dynamicEmergencyContactName,
      emergencyContactPhone: dynamicEmergencyContactPhone,
      licenseNumber: dynamicLicenseNumber,
